...
/*   Updated: 2018/02/16 15:03:54 by bpajot      ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
t_field			*ft_type3(t_field *current, const char *str, int i, va_list *va)
{
	...
	if (str[i] == 'n')
		current->type = N_FLAG;
	val = va_arg(*va, long int);
//	printf("val = %d\n", val);
	if (val < 0L && ((current->type == C_MAJ) || (current->type == C_MIN &&
		current->conv == L_FLAG)))
	{
		current->error = 1;
//		printf("error 1");
//		printf("val = %d\n", val);
	...
	if (val > 255L && MB_CUR_MAX == 1 && ((current->type == C_MAJ) ||
...
//		printf("error 2");
	...
	if (val > 0x10ffffL && (current->type == C_MAJ || (current->type == C_MIN &&
		current->conv == L_FLAG)))
	{
		current->error = 1;
//		printf("error 3");
//		printf("val = %d\n", val);
	...
	if (val > 0xd7ffL && val < 0xe000L && (current->type == C_MAJ ||
		(current->type == C_MIN && current->conv == L_FLAG)))
	{
		current->error = 1;
//		printf("error 4");
